...
--
-- Dumping events for database 'movie_streaming'
--

--
-- Dumping routines for database 'movie_streaming'
--
/*!50003 DROP FUNCTION IF EXISTS `get_usercount` */;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addmovies`(
				IN movie_title VARCHAR(255),-- this will be movie_title VARCHAR(255)
                IN movie_release_date DATE,-- this will be movie_release_date DATE
                IN movie_duration TIME, -- movie_duration TIME
                IN movie_description VARCHAR(255),-- movie_description VARCHAR(255)
                OUT new_movie_id INT)
BEGIN -- the blovk where the insertion happens
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addSubscription`(
				IN user_id INT,-- this will be input, user id
                IN start_date DATE,-- this will be movie_release_date DATE
                IN end_date DATE, -- input end_date date
                IN new_subscription_status ENUM('Active','Inactive'),-- ENUM will take only supported values
                OUT new_subscription_id INT)
BEGIN -- the block where the insertion happens
...
-- Dump completed on 2024-11-28 19:50:06
